 ...
 *   2. Create project "Discover Colorado"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Colorado Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'colorado'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Colorado ${year}`,
      description: `${title}\n\nFull guide: https://discovercolorado.info/${slug}/`,
      tags: [`Colorado ${slug}`, 'Colorado travel', `Colorado ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Colorado ✈️ #shorts`,
      description: `${title} — Full guide: https://discovercolorado.info/destinations/${slug}/`,
      tags: [`${title} Colorado`, 'Colorado travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Colorado — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Colorado for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discovercolorado.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discovercolorado.info/companion/

#Colorado #${title.replace(/\s+/g, '')} #ColoradoTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    `${title} Colorado`,
    `${title} travel guide`,
    `Colorado ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Colorado travel',
    'Colorado food',
    'Colorado budget travel',
    'Colorado tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Colorado`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER COLORADO               ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
